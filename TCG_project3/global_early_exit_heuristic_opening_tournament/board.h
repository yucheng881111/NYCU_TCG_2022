...
public:
	...
	reward test(int x, int y, unsigned who = piece_type::unknown) const {
		if (who == -1u) who = attr.who_take_turns;
		// if (who != attr.who_take_turns) return nogo_move_result::illegal_turn;
		if (x == -1 && y == -1) return nogo_move_result::illegal_pass;
		point p_min(0, 0), p_max(size_x - 1, size_y - 1);
	...
	class undo {
		friend class board;
	private:
		undo(board* b) : b{b}, placed{false} {}
	public:
		~undo() {
			if (placed) {
				b->stone[x][y] = who;
				b->attr.who_take_turns = static_cast<piece_type>(3u - b->attr.who_take_turns);
			}
		}
	public:
		void place(int x, int y, unsigned who) {
			placed = true;
			// backup
			this->x = x; this->y = y; this->who = b->stone[x][y];
			// place
			unsigned opp = 3u - who;
			b->stone[x][y] = who;
			b->attr.who_take_turns = static_cast<piece_type>(opp);
		}
	private:
		board* b;
		bool placed;
